...
## Contributing Assets to Portage on AWS

Thank you for considering a contribution to Portage on AWS project!
...
1. Contributing Portage on AWS Templates
   1. Provider Template
   2. App Template
   3. Resource Template
   4. Other Template
2. Contributing Portage on AWS Pipelines
   1. New pipeline pattern
   2. Update existing pipeline pattern
3. Contributing Portage on AWS Core modifications
   1. Change in UI / Frontend of Portage on AWS plugins
   2. Change in SDK API / Backend of Portage on AWS Plugins
   3. Change in sacffolder actions
   4. Change in architecture or platform design
   5. Change in common interfaces
4. Contributing Portage on AWS extensions
   1. Integration with new tools and plugins
   2. Integration with additional Backstage.io APIs / entities
   3. Extending Portage on AWS model

### Submitting Contribution

Before submitting any contribution type please make sure it adheres to the Portage on AWS [architecture](https://portage.digi-trans.org/docs/techdocs/architecture)

### Contributing an Portage on AWS provider
...
We highly recommend to test different context for this step as described in the [test-cases](https://portage.digi-trans.org/docs/tests)

**Step 2**<br/>
Make sure you can update the provider configurations or IAC and the pipeline will apply the changes succufully 

**Step 3**<br/>
Add entries in  [test-cases](https://portage.digi-trans.org/docs/tests) document for the new provider implemented.

#### Submit your PR
Submit a pull request for the new provider following the instructions in this page.


### Contributing an Portage on AWS application
...
3. Does this application can use an existing environment or it requires a new environment type / provider? if so, see the above *Contributing an Portage on AWS provider*
...
**Step 3**<br/>
Add entries in  [test-cases](https://portage.digi-trans.org/docs/tests) document for the new application implemented.
